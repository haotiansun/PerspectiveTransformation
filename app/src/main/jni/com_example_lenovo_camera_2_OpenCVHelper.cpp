...
#include "com_example_lenovo_camera_2_OpenCVHelper.h"
#include <stdio.h>
#include <stdlib.h>
...
JNIEXPORT jintArray JNICALL Java_com_example_lenovo_camera_12_OpenCVHelper_gray(
    ...
    //copy from scan
    Mat img_proc;
    int w1 = img.size().width, h1 = img.size().height, min_w = 200;
    double scale = min(10.0, w1 * 1.0 / min_w);
    int w_proc = w1 * 1.0 / scale, h_proc = h1 * 1.0 / scale;

    resize(img, img_proc, Size(w_proc, h_proc));

    Mat img_dis = img_proc.clone();
    //GaussianBlur(img_dis, img_proc, Size(5, 5), 0, 0);
    //pyrMeanShiftFiltering(img_dis, img_proc, 25, 10, 1);
    cv::cvtColor(img_dis, img_dis, CV_RGBA2RGB, 3);
    cv::cvtColor(img_proc, img_proc, CV_RGBA2RGB, 3);
    bilateralFilter(img_dis, img_proc, 25, 25 * 2, 25 / 2);
    //   namedWindow("dst1", WINDOW_NORMAL);
    //   imshow("dst1", img_proc);
    ...
    HoughLinesP(canny, lines, 1, CV_PI / 180, w_proc / 5, w_proc / 5, 20);
...
//    int w_a4 = 1654, h_a4 = 2339;
    ...
    Mat dst = Mat::zeros(h, w, CV_8UC4);

    // corners of destination image with the sequence [tl, tr, bl, br]
    vector<Point2f> dst_pts, img_pts;
    dst_pts.push_back(Point(0, 0));
    dst_pts.push_back(Point(w - 1, 0));
    dst_pts.push_back(Point(0, h - 1));
    dst_pts.push_back(Point(w - 1, h - 1));
    ...
    warpPerspective(img, img, transmtx, img.size());
    ...
    if (debug) {
        ...
        imshow("src", img_dis);
        imshow("canny", canny);
        imwrite("C://Users//Lenovo//Desktop//display//canny.jpg", canny);
        imshow("img_proc", img_proc);
        imwrite("C://Users//Lenovo//Desktop//display//img_proc.jpg", img_proc);
        imshow("dst", dst);
        imwrite("C://Users//Lenovo//Desktop//display//dst.jpg", dst);
        waitKey(0);
         */
        imwrite("/storage/extSdCard/DCIM/Camera/13.jpg",dst);
    }
...
//    Mat img1 = imread("/storage/extSdCard/DCIM/Camera/13.jpg");
//    scan("/storage/extSdCard/DCIM/Camera/13.jpg");
//    if (!img1.empty()) {
//        imshow("/storage/extSdCard/DCIM/Camera/13.jpg", img1);
//    }
    //imshow("/storage/extSdCard/DCIM/Camera/13.jpg", img1);
    int size = w * h;
